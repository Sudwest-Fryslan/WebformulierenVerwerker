...
de Atabix-formulierbeheerder/Atabix, de Doorstroommedewerker/CAReL-beheer of Eljakim liggen — dat staat in `docs/carel/scopedocument.md`).
...
| 7 | CAReL `heeftBetrekkingOp` / `verwerkingssoort` | **Ja** | ✅ Opgelost | Gedaan | Check bij de Doorstroommedewerker/Eljakim nog nodig (geen code-actie) |
...
adres-splitsing + dagstructuur — wacht op nieuwe formuliervelden van de Atabix-formulierbeheerder, en op afstemming met Eljakim
...
SOAP Fault). Zie sectie 7 voor details. Diezelfde dag heeft Eljakim (de CAReL-leverancier (Eljakim)) drie
...
alle 8 operaties, `diff` toonde precies 1 regel verschil), maar op verzoek van de Solution Innovator (ontwikkelaar) teruggedraaid: te
...
**Update 19 augustus 2026 — definitieve lijst na terugkoppeling de CAReL-leverancier (Eljakim) (Eljakim, via de Doorstroommedewerker):** de CAReL-leverancier (Eljakim)
...
1. **Adres-splitsing** — expliciet bevestigd door de CAReL-leverancier (Eljakim) voor zowel het leerling- als het
   ...
   deze velden voor leerling/school ook los uitvraagt (vraag aan de Atabix-formulierbeheerder).
2. **Volledige keuzelijsten voor de twee bekende mismatch-velden** — `vervoer_type` en de
   ...
   volledige lijst mogelijke antwoorden). Vraag aan de Atabix-formulierbeheerder: welke keuzeopties bestaan er per veld.

- **Zelf te doen?** De adres-splitsing technisch ja, zodra bekend is welke formuliervelden er zijn. De
  vervoer-/dagstructuur-mapping ook zelf te bouwen zodra de volledige optielijst van de Atabix-formulierbeheerder binnen is —
...
**Gedaan, op verzoek van de Solution Innovator (ontwikkelaar):**
...
BSN 111222333 via `heeftBetrekkingOp`, aanvrager BSN 444555666 met volledige gegevens via
...
1. ~~Of CAReL's eigen software akkoord gaat met "BSN + basisgegevens"~~ — **bijgesteld, 24 juli 2026:**
   Eljakim (de CAReL-leverancier (Eljakim)) meldt dat CAReL het BSN prima herkent en zelf een BRP-bevraging
   ...
   moet de CAReL-applicatiebeheerder/Eljakim nog in CAReL zelf controleren voor zaak 1900881353).
...
- **Gesprek met WeAreFrank?** Niet nodig geweest voor de implementatie zelf — wel nog een check bij
  de Doorstroommedewerker/Eljakim nodig of de gekozen aanpak (BSN-plus, geen volledig adres) door CAReL geaccepteerd wordt.
